...
class Mirasvit_MstCore_Helper_Validator extends Mirasvit_MstCore_Helper_Validator_Abstract
{
    public function devTestCache()
    {
        ...
        foreach (Mage::helper('mstcore')->getUsedCaches() as $name => $method) {
            ...
            ' <a style="cursor:pointer" href="' . Mage::helper('adminhtml')->getUrl('adminhtml/validator/clearCache', array('cache_method' => $method)) . '">Flush Cache</a>';
            $result = self::INFO;
        }
...
redis> flushall
    ...
    public function devTestCompilation()
    {
        ...
        if (defined('COMPILER_INCLUDE_PATH')) {
            $result = self::INFO;
            $description[] = 'Compilation status: Enabled, <a href="' . Mage::helper('adminhtml')->getUrl('adminhtml/validator/compiler', array('action' => 'disable')) . '">Disable</a>';
        }
