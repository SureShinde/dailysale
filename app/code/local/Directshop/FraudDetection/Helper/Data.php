...
class Directshop_FraudDetection_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	function normaliseMaxMindResponse($res)
	{
		...
		{
			$res['ourscore'] = round(floatval($res['riskScore']),0);
		}
		else if (isset($res['score']))
		{
			$res['ourscore'] = round((floatval($res['score']) * 10),0);
		}
		else
		{
			//$res['ourscore'] = -1;
			$res['err'] = $res['err'] or "FATAL_ERROR";
			$res['errmsg'] = $res['errmsg'] or "No Maxmind Response Found";
		}
		return $res;
	...
	function getMaxMindResponse($payment)
	{
		...
		// Required fields
		$h["i"] = $order->getRemoteIp();             // set the client ip address
