 ...
 * @author Mihail
 */
class Fiuze_Soapapi_Adminhtml_SoapapiController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Init actions
     *
    ...
    protected function _initAction()
    {
        ...
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                  Mage::helper('fiuze_soapapi')->__('Fiuze'),
                  Mage::helper('fiuze_soapapi')->__('Fiuze')
              )
            ->_addBreadcrumb(
                  Mage::helper('fiuze_soapapi')->__('ImportProducts'),
                  Mage::helper('fiuze_soapapi')->__('ImportProducts')
              )
        ...
        if ($data) {
            ...
            try {
                if (isset($_FILES['importfile']['name']) && (file_exists($_FILES['importfile']['tmp_name']))) {
                    ...
                    $fileHelper = Mage::helper('fiuze_soapapi/file');
                    ...
                    if($importContent) {
                        try{
                            ...
                            $soapclient = Mage::helper('fiuze_soapapi/data')->getSoapClient();
                            $apiuser = Mage::helper('fiuze_soapapi/data')->getApiUser();
                            $apikey = Mage::helper('fiuze_soapapi/data')->getApiKey();
                            ...
                            $result = Mage::helper('fiuze_soapapi/api')->createConfigurable_v2($proxy, $sessionId);
                            ...
                            foreach($importContent->configProductId as $configurableProductId) {
                                ...
                                $result = Mage::helper('fiuze_soapapi/api')->linkAssign_v2($proxy, $sessionId, (string)$importContent->operation[$confSp], (string)$configurableProductId, $linkedIds);                                            
                        ...
                        $hasError = true;
                        $this->_getSession()->addError(Mage::helper('fiuze_soapapi')->__('Loading products is failed'));
                    }
                } else {
                    $hasError = true;
                    $this->_getSession()->addError(Mage::helper('fiuze_soapapi')->__('File upload is failed'));
                }
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('fiuze_soapapi')->__('An error occurred while importing the file.')
                );
            }
            ...
            if ($hasError) {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('fiuze_soapapi')->__('Successfully imported.')
                );
            }
        }
    ...
    */
    ...
    {
        if (Mage::helper('fiuze_soapapi/file')->flushImagesCache()) {
