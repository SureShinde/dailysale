...
class Unirgy_DropshipPayout_Udpayoutadmin_PayoutController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('udropship')->__('Payouts'), Mage::helper('udropship')->__('Payouts'));
                ...
                if (($id = $this->getRequest()->getParam('id')) > 0) {
                    if (($payout = Mage::getModel('udpayout/payout')->load($id)) && $payout->getId()) {
                        ...
                        if ($this->getRequest()->getParam('pay_flag')) {
                            $payout->pay();
                            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('udropship')->__('Payout was successfully paid'));
                        }
                    } else {
                        Mage::throwException(Mage::helper('udropship')->__("Payout '%s' no longer exists", $id));
                    }
                } else {
                    $hlp->processPost();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('udropship')->__('Payout was successfully saved'));
    ...
    public function deleteAction()
    {
        if (($id = $this->getRequest()->getParam('id')) > 0 ) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('udropship')->__('Payout was successfully deleted'));
                $this->_redirect('*/*/');
            } catch (Exception $e) {
                Mage::logException($e);
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($payoutIds)) {
            $this->_getSession()->addError(Mage::helper('udropship')->__('Please select payout(s)'));
        }
        else {
            try {
                $updatedCnt = 0;
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('udropship')->__('%s of %d record(s) were successfully deleted', $updatedCnt, count($payoutIds))
                );
            } catch (Exception $e) {
                Mage::logException($e);
    ...
    public function massStatusAction()
    {
        ...
        try {
            ...
            $this->_getSession()->addSuccess(
                Mage::helper('udropship')->__('%s of %d record(s) were successfully updated', $updatedCnt, count($payoutIds))
            );
        }
            ...
            $this->_getSession()->addException($e, Mage::helper('udropship')->__('There was an error while updating payout(s) status'));
        }
    ...
    public function massPayAction()
    {
        ...
        try {
            ...
            $this->_getSession()->addSuccess(
                Mage::helper('udropship')->__('%s of %s payouts  were successfully paid', $paidCnt, count($modelIds))
            );
        }
        catch (Mage_Core_Model_Exception $e) {
            Mage::logException($e);
            ...
            Mage::logException($e);
            $this->_getSession()->addException($e, Mage::helper('udropship')->__('There was an error during payout(s) mass pay'));
        }
