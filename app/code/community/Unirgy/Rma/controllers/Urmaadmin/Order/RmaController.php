...
class Unirgy_Rma_Urmaadmin_Order_RmaController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _initRma($forSave=true)
    {
        ...
        if ($rmaId) {
            ...
            if (!$rma->getId()) {
                Mage::throwException(Mage::helper('udropship')->__('This Return no longer exists.'));
            }
        } elseif ($orderId) {
    ...
    public function saveAction()
    {
        try {
            $this->_saveRma();
            $this->_getSession()->addSuccess(Mage::helper('udropship')->__('The Return has been created.'));
    ...
    public function newAction()
    {
        if ($rma = $this->_initRma(false)) {
            $this->_title(Mage::helper('udropship')->__('New Return'));
    ...
    {
        if ($rma = $this->_initRma(false)) {
            $this->_title(Mage::helper('udropship')->__('View Return'));
    ...
    public function createLabelAction()
    {
        try {
            ...
            if (empty($data['generate_label'])) {
                Mage::throwException(Mage::helper('udropship')->__('Wrong generate label request.'));
            }
            if ($rma = $this->_initRma(false)) {
                $labelData = array();
                ...
                if ($batch->getShipmentCnt() && $batch->getLastTrack()) {
                    ...
                    if ($batch->getErrors()) {
                        ...
                        foreach ($batch->getErrors() as $error=>$cnt) {
                            $errs[] = Mage::helper('udropship')->__($error, $cnt);
                        }
                ...
                $response = array(
                    'error'     => true,
                    'message'   => Mage::helper('udropship')->__('Cannot initialize rma for adding tracking number.'),
                );
            }
        } catch (Mage_Core_Exception $e) {
            $response = array(
            ...
            $response = array(
                'error'     => true,
                'message'   => Mage::helper('udropship')->__('Cannot add tracking number.'),
            );
        }
    ...
    public function addTrackAction()
    {
        try {
            ...
            if (empty($carrier)) {
                Mage::throwException(Mage::helper('udropship')->__('The carrier needs to be specified.'));
            }
            if (empty($number)) {
                Mage::throwException(Mage::helper('udropship')->__('Tracking number cannot be empty.'));
            }
            if ($rma = $this->_initRma(false)) {
        ...
        if ($track->getId()) {
            try {
                if ($rmaId = $this->_initRma(false)) {
                    ...
                    $response = array(
                        'error'     => true,
                        'message'   => Mage::helper('udropship')->__('Cannot initialize rma for delete tracking number.'),
                    );
                }
            } catch (Exception $e) {
                $response = array(
                    'error'     => true,
                    'message'   => Mage::helper('udropship')->__('Cannot delete tracking number.'),
                );
            }
        } else {
            $response = array(
                'error'     => true,
                'message'   => Mage::helper('udropship')->__('Cannot load track with retrieving identifier.'),
            );
        }
        if (is_array($response)) {
            $response = Mage::helper('core')->jsonEncode($response);
    ...
    public function viewTrackAction()
    {
        ...
        if ($track->getId()) {
            try {
                ...
                $response = array(
                    'error'     => true,
                    'message'   => Mage::helper('udropship')->__('Cannot retrieve tracking number detail.'),
                );
            }
        } else {
            $response = array(
